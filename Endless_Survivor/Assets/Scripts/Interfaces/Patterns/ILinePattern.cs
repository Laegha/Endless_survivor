...
public class ILinePattern : IPattern
{
    ...
    public IEnumerable<Vector2> GetPositions(Vector2 origin, int count)
    {
        ...
        Vector2 startPos = origin - new Vector2(_hAlignment == HorizontalAlignment.Center ? newSpacing.x * (count -1) / 2 : 0, _vAlignment == VerticalAlignment.Center ? newSpacing.y * (count - 1) / 2 : 0);
